...
export class MotionBlindsAccessory {
  ...
  constructor(
    ...
    this.service
      ...
      .onGet(() => { // Use modern onGet
        ...
        this.platform.log.debug(`<- get CurrentPosition (${this.mac}): blind=${currentPosition}, hk=${homekitPosition} (inverted=${!!this.config.invert})`); // Keep original debug log
    ...
    if (this.platform.gateway.key) {
      ...
      this.service
        ...
        .onSet(async (value) => { // Use modern onSet with async
          ...
          this.platform.log.debug(`-> set TargetPosition (${this.mac}): hk=${targetPosition}, blind=${effectiveTarget} (inverted=${!!this.config.invert})`); // Keep original debug log
          try {
      ...
      if (this.config.tilt) {
        ...
        this.service
          ...
          .onGet(() => {
            ...
            this.platform.log.debug(`<- get CurrentTiltAngle (${this.mac}): blind=${this.status.currentAngle}, hk=${currentAngle}`);
            return currentAngle;
          });
      }
    } else {
          ...
          this.platform.gateway
            ...
            .then((res) => {
              this.platform.log.debug(
                `<- Polling readDevice(${this.mac}, ${this.deviceType}) => ${JSON.stringify(res.data)}`,
              );
              this.updateAccessory(res.data);
            })
            .catch((err) => {
              this.platform.log.debug(`Polling readDevice(${this.mac}, ${this.deviceType}) failed:`, err.message);
  ...
  updateAccessory(newStatus: DeviceStatus, isInitialUpdate = false) {
    ...
    this.platform.log.debug(`  Blind Status: pos=${newStatus.currentPosition}, angle=${newStatus.currentAngle}, op=${Operation[newStatus.operation]}, batt=${newStatus.batteryLevel}`);
    this.platform.log.debug(`  HK Status:    pos=${newHomeKitPosition}, state=${newHomeKitState}, angle=${newHomeKitAngle}, battLvl=${newBatteryLevel}, battStat=${newBatteryStatus}`);
